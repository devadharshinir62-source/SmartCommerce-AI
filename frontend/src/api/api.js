 ...
 * =================================
 * Connects frontend React components to FastAPI backend endpoints at http://localhost:8000.
 */
 ...
 */
export async function fetchProducts({ category, search, maxPrice, minRating } = {}) {
 ...
 * e.g., "I need a laptop for coding under ₹60,000"
 */
 ...
 */
export async function sendChatMessage(message, history = [], contextProductId = null) {
  ...
  if (!response.ok) {
    throw new Error(`Chat assistant failed: ${response.statusText}`);
  }
  return await response.json();
}
